...
> **Secure Password Generator & Vault** is a lightweight, strictly offline desktop application designed to solve password fatigue. Built with Python and PyQt6, it combines cryptographically secure password generation with a bank-level AES-encrypted local SQLite vault, giving you complete, cloud-free control over your digital security.
...
*(Note: When you save your first password, the app will generate `secret.key` and `vault.db` files in the same folder. Keep them together with the `.exe` file!)*
...
It is the master key to your vault. If this file is lost, the encrypted passwords inside `vault.db` cannot be recovered by anyone.
```
## 📄 License
